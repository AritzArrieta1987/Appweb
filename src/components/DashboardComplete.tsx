...
export default function DashboardComplete({ onLogout }: DashboardProps) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'Dashboard':
        return (
          <div style={{ padding: '0' }}>
            {/* Contenido limpio sin título ni cajas */}
          </div>
        );
      ...
      case 'Contratos':
        return (
          <div style={{ padding: '0' }}>
            ...
            <p style={{ color: '#AFB3B7' }}>Gestión de contratos (Próximamente)</p>
          </div>
        );
  ...
  if (selectedArtist) {
    // Filtrar los tracks del artista seleccionado
    const artistTracks = tracks.filter(track => track.artistName === selectedArtist.name);
    ...
    return (
      <ArtistPanel
        artist={{ ...selectedArtist, trackCount: artistTracks.length }}
        tracks={artistTracks}
